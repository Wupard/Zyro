  ...
  db.collection('systemAnnouncements')
    ...
    .onSnapshot(snap => {
      ...
      docs.forEach(({ id, data }) => {
        ...
        const deleteBtn = isAdmin ? `<button class="updates-delete-btn" onclick="adminDeleteUpdate('${id}')" title="Sil" style="background:rgba(239,68,68,0.05);border:1px solid rgba(239,68,68,0.15);color:#ef4444;cursor:pointer;padding:6px 8px;border-radius:8px;transition:all 0.2s;display:flex;align-items:center;justify-content:center;" onmouseenter="this.style.background='rgba(239,68,68,0.15)';this.style.borderColor='rgba(239,68,68,0.3)'" onmouseleave="this.style.background='rgba(239,68,68,0.05)';this.style.borderColor='rgba(239,68,68,0.15)'">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>` : '';

        const editBtn = isAdmin ? `<button class="updates-edit-btn" onclick="adminEditUpdate('${id}')" title="Düzenle" style="background:rgba(139,124,247,0.05);border:1px solid rgba(139,124,247,0.15);color:var(--accent-primary);cursor:pointer;padding:6px 8px;border-radius:8px;transition:all 0.2s;display:flex;align-items:center;justify-content:center;" onmouseenter="this.style.background='rgba(139,124,247,0.15)';this.style.borderColor='rgba(139,124,247,0.3)'" onmouseleave="this.style.background='rgba(139,124,247,0.05)';this.style.borderColor='rgba(139,124,247,0.15)'">
      ...
      if (err.code === 'failed-precondition') {
        db.collection('systemAnnouncements').get().then(snap => {
          ...
          docs.forEach(({ id, data }) => {
            ...
            const dateStr = createdAt.toLocaleDateString('tr-TR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
            const timeStr = createdAt.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
            const deleteBtn = isAdmin ? `<button class="updates-delete-btn" onclick="adminDeleteUpdate('${id}')" title="Sil" style="background:rgba(239,68,68,0.05);border:1px solid rgba(239,68,68,0.15);color:#ef4444;cursor:pointer;padding:6px 8px;border-radius:8px;transition:all 0.2s;display:flex;align-items:center;justify-content:center;" onmouseenter="this.style.background='rgba(239,68,68,0.15)';this.style.borderColor='rgba(239,68,68,0.3)'" onmouseleave="this.style.background='rgba(239,68,68,0.05)';this.style.borderColor='rgba(239,68,68,0.15)'"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>` : '';
            const editBtn = isAdmin ? `<button class="updates-edit-btn" onclick="adminEditUpdate('${id}')" title="Düzenle" style="background:rgba(139,124,247,0.05);border:1px solid rgba(139,124,247,0.15);color:var(--accent-primary);cursor:pointer;padding:6px 8px;border-radius:8px;transition:all 0.2s;display:flex;align-items:center;justify-content:center;" onmouseenter="this.style.background='rgba(139,124,247,0.15)';this.style.borderColor='rgba(139,124,247,0.3)'" onmouseleave="this.style.background='rgba(139,124,247,0.05)';this.style.borderColor='rgba(139,124,247,0.15)'"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>` : '';
            html += `<section class="card updates-card" style="margin-bottom:16px;"><div class="updates-head"><div class="updates-head-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v4"/><path d="M12 18v4"/><path d="M4.93 4.93l2.83 2.83"/><path d="M16.24 16.24l2.83 2.83"/><path d="M2 12h4"/><path d="M18 12h4"/><path d="M4.93 19.07l2.83-2.83"/><path d="M16.24 7.76l2.83-2.83"/></svg></div><div style="display:flex;flex-direction:column;gap:4px;min-width:0;flex:1;"><div class="updates-title">Geliştirme Özeti</div><div class="updates-subtitle">Yapımcı: Wupard</div></div><div style="display:flex;align-items:center;gap:6px;">${editBtn}${deleteBtn}</div></div><div class="updates-timeline"><div class="updates-entry"><div class="updates-entry-marker"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg></div><div class="updates-entry-body"><div class="updates-entry-date">${dateStr.toUpperCase()} — ${timeStr}</div><div id="update-content-${id}"><div class="updates-content-parsed">${parseUpdateContent(items)}</div></div></div></div></div></section>`;
          });
          container.innerHTML = html;
